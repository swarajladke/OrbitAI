...
| Participation form complete | PASS submitted / FAIL resubmission outstanding | Submitted 2026-09-01 19:34. The Challenge Admin acknowledged the proposal on 2026-09-03 and requested a refined resubmission addressing team capacity and prior work, due 9 September 2026. The executive summary, the technology-domain selections and the referral question have since been corrected on the live form. The Docker image link, the proposal re-upload and the maturity level are still to be confirmed. |
...
| Tracking (not just per-window detection) | FAIL | `src/tracker.py` implements cross-window association and ships inside the image, but it is not called by `src/infer.py`, `src/pipeline.py` or `run.sh`, and no file in `src/` reads the `tracking_mode` key declared in `config.yaml`. Measured 2026-09-06 by occurrence count across all 39 files in `src/`. The submitted path emits per-window detections with no persistent identity, and the 9-field prediction rows carry no track column. Temporal information is used to suppress noise, through two-window persistence matching and `min_hits` filtering, but no track identity is produced. See 16.2 item 10. |
...
| p99 and maximum per-window latency under 40 ms (not just the mean) | FAIL | Measured 2026-09-06 on the 17 training sequences, 5 independent repetitions, 20 warmup windows excluded. Compute p99 is under 40 ms on 15 of 17. Maximum per-window latency is under 40 ms on 2 of 17 and ranges from 18.65 ms to 704.16 ms. Total latency, defined as compute plus the one-window 40 ms lookahead, exceeds 40 ms on 17 of 17. See 16.2 items 2, 3, 12 and 13. |
| Latency benchmark exercises the submitted algorithm | FAIL | `src/latency_bench.py` contains zero references to `box_regressor` and zero to `reg_w`, and passes `count_img=None`, so the benchmarked path omits the Arm 2 box regressor entirely and scores candidates with `local_bg` fixed at 0.0. `src/pipeline.py` references `box_regressor` six times and `local_bg` three times. Measured 2026-09-06 by occurrence count. Every latency figure in this document therefore describes a near neighbour of the shipped pipeline rather than the shipped pipeline itself. See 16.2 item 11. |
...
| Tracking emitted no persistent identities | STILL OPEN, reopened 2026-09-06 | The 2026-09-05 entry claimed closure from file presence rather than from a call-path check. `src/tracker.py` exists and is used by `src/train_reranker.py`, but not by the submitted inference path, and `tracking_mode` is read by no source file. |
...
| Latency was a single mean on unstated hardware | PARTIALLY CLOSED | p99 and maximum per-window latency are both now measured, over 5 repetitions with 20 warmup windows, on the 17 training sequences, 2026-09-06. Evaluation-hardware equivalence and benchmark-to-pipeline parity remain open. |
...
2. **Two training sequences exceed the latency budget at compute p99,** measured 2026-09-06: `DVX_NOAA6_11416` at 137.74 +/- 52.35 ms, with 5 windows isolated as system stalls above 1000 ms, and `2025_12_23_21_12_28_EVK4_mag5.2` at 61.90 +/- 3.96 ms. The other 15 pass. The figures of 84.67, 77.76 and 58.73 ms recorded on 2026-09-05 came from a run across all 21 sequences and are superseded for the training split. Per-sensor decimation remains the untested next lever.
3. **Maximum per-window latency is now measured, and mostly fails.** Under 40 ms on 2 of 17 training sequences, at 18.65 +/- 2.0 ms and 31.17 +/- 3.9 ms. The other 15 range from 52.02 ms to 704.16 ms. The maximum is jitter-dominated rather than algorithmic: `DAVIS_EGS_16908` shows a compute p99 of 21.47 +/- 0.12 ms against a maximum of 159.94 +/- 86.80 ms, a run-to-run spread on the maximum roughly four times the p99 itself. On a general-purpose preemptive operating system the per-window maximum is not a stable property of the algorithm. It is reported here rather than suppressed, and no per-window maximum is offered as a guarantee.
...
10. **Tracking is implemented but not integrated.** `src/tracker.py` is not on the submitted inference path and `tracking_mode` is dead configuration. Section 3 of the official text asks for detection, tracking and visualization. The submitted entrypoint delivers detection, uses temporal persistence for noise suppression rather than for identity, and delivers visualization only as an offline renderer. Integrating identity assignment after final detection would require a rebuild, a re-run and a new release, and is not attempted before the deadline. The prediction schema is fixed at 9 official fields and must not gain a tenth column, so any identity output would have to be a sidecar file.
11. **The latency benchmark and the inference pipeline are not the same code path.** `src/latency_bench.py` omits the Arm 2 box regressor and the `local_bg` feature. The correct fix is shared stage functions called by both paths, with a parity assertion on features and predictions before any timing is trusted. Not attempted before the deadline, because it would invalidate every published latency figure without leaving time to regenerate them.
12. **End-to-end latency exceeds 40 ms on every sequence by definition.** The pipeline consumes one window of lookahead, so total latency is compute plus 40 ms and cannot be under 40 ms for any sequence. Section 9 of the official text asks for latency under 40 ms end-to-end. The figure claimed in the proposal and on the portal is compute p99 under 40 ms, a different quantity, labelled as such in proposal section 2.3. A genuinely causal variant was measured and cost 16.1 percent of overall mAP and 48.0 percent on sparse sequences, against a pre-Arm-2 baseline.
13. **Reported latency is sensitive to the warmup-window count.** The same two sequences measured with 5 warmup windows and 1 repetition gave compute p99 of 102.98 ms and 66.35 ms. With 20 warmup windows and 5 repetitions they gave 61.90 ms and 27.02 ms. Cold caches and first-call overhead in the tree predictors dominate the early windows. Every latency figure in this document is therefore quoted with its repetition and warmup counts attached, and no figure should be compared against one measured under different settings.
14. **The shipped model files were pickled by a newer scikit-learn than the container installs.** Loading the three joblib files raises `InconsistentVersionWarning`: they were written by scikit-learn 1.9.0, while `requirements.txt` pins 1.5.1. The submitted container therefore loads its own weights under a version mismatch. The published metrics were produced under exactly this configuration and verified against the official `evaluate.py`, so they are internally consistent, but the mismatch is undisclosed fragility and it matters for the section 11 reproducibility package, where training under 1.5.1 is not guaranteed to reproduce weights created under 1.9.0. Not changed before the deadline, because repinning would require rebuilding and re-releasing an already verified image.
